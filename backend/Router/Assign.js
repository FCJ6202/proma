...
router.post('/create/:RepoId',fetchData, [body('title', 'please input valid text').isLength({ min: 4 }),
body('Type', 'please enter atleast 4 length type').isLength({ min: 4 })] ,async (req,res) => {
    ...
    try {
        const {text,Type,title,materials,dueDate,dueTime,points} = req.body
        ...
        const assignData = new assign({
            RepoId,text,CreaterName : CreaterData.name,CreaterUserId,Type,title,materials,dueDate,dueTime,points
        })

        // Update Annouce Id in Repo Database
        RepoData.Annoucement.push(annouceData.id);
        RepoData.Assignment.push(assignData.id);
...
router.post('/fetchdata/:RepoId',fetchData ,async (req,res) => {
    ...
    try {
        ...
        for(var i=0;i<RepoData.Assignment.length;i++){
                const temp = await assign.findById(RepoData.Assignment[i]);
                //console.log(temp)
                AssignArray.push(temp);
        }

        res.json(AssignArray);
